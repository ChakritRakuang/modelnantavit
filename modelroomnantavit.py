...
for at, v in zip(['color1', 'color2', 'color3', 'fillerColor'],
                 [(0.08, 0.097, 0.0915), (0.17, 0.21, 0.24), (0.88, 1.0, 0.999), (0.59, 0.59, 0.59)]):
    mc.setAttr(lai_phuen + '.' + at, *v, typ='double3')
for at, v in zip(['cellSize', 'density', 'mixRatio', 'spottyness', 'randomness', 'threshold'],
                 [10, 0.8, 0.5, 0.1, 1, 0.8]):
...
for at, v in zip(['color1', 'color2', 'color3', 'fillerColor'],
                 [(0.047, 0.057, 0.054), (0.14, 0.17, 0.19), (0.947, 1, 1), (0.36, 0.36, 0.36)]):
...
# หน้าต่าง
krop = [mc.polyCube(w=6, h=1, d=152, sx=3)[0], mc.polyCube(w=6, h=8, d=152, sx=3)[0]]
mc.move(-3, 0.5)
mc.move(-3, 89 + 4)
for i in [0, 1]:
...
mc.select(cl=1)
kho = [mc.joint(p=[0, 0, 0], n='kraduk_natang'), mc.joint(p=[-1.9, 47, -76 / 2. + 2 + 0.5]),
       mc.joint(p=[-1.9, 47, -20 + 2 + 0.5]), mc.joint(p=[-3.9, 47, 76 / 2. - 2 - 0.5]),
       mc.joint(p=[-3.9, 47, 20 - 2 - 0.5]), mc.joint(p=[-1.9, 120, -76 / 2. + 2 + 0.5]),
       mc.joint(p=[-1.9, 120, -20 + 2 + 0.5]), mc.joint(p=[-3.9, 120, 76 / 2. - 2 - 0.5]),
       mc.joint(p=[-3.9, 120, 20 - 2 - 0.5])]
mc.select(kho[0])
mc.select(kho[0])
mc.select(kho[0])
...
mc.select(cl=1)
kraduk_pratu = [mc.joint(p=[430 - 2, 0, 170], n='kraduk_pratu'), mc.joint(p=[430 - 2, 185 / 2., 211]),
                mc.joint(p=[430 - 2, 185 / 2., 129])]
...
mc.delete(kan1 + '.f[1:3]', kan2 + '.f[2]', kan2 + '.f[4:5]', kan3 + '.f[2]', kan3 + '.f[4:5]', kan4 + '.f[1]',
          kan4 + '.f[4:5]')
...
chut = [(-22, 83, 28), (-22, 72, 26), (-22, 52, 21), (-22, 44, 18), (-22, 42, 15), (-22, 42, -20), (-19, 42, -24),
        (-14, 42, -25)]
...
# เตียง
tiang = [mc.polyCube(w=187, h=35, d=2)[0], mc.polyCube(w=187, h=14, d=2)[0], mc.polyCube(w=2, h=14, d=82)[0],
         mc.polyCube(w=2, h=35, d=82)[0], mc.polyCube(w=196, h=2, d=91)[0]]
...
for a in [phiu_thinon1, phiu_thinon2]:
    ...
    for i in range(mc.polyEvaluate(v=1)):
        ...
        t = mc.xform(s, q=1, t=1)
        mc.scale(
            (((32.5 ** 2 - t[0] ** 2) * (20 ** 2 - t[2] ** 2)) ** 0.5 / 32.5 / 20 + 0.1 / 7.5) * random.uniform(0.9,
                                                                                                                1.1), s,
            y=1, p=[0, 0, 0])
        mc.move(random.uniform(-0.5, 0.5), 0, random.uniform(-0.5, 0.5), s, r=1)
        if t[1] < ymin:
...
for i, tsp in zip([1, -1], tusueapha):
    mc.select(cl=1)
    kho = [mc.joint(p=[0, 0, 0], n='kraduk_tusueapha1'), mc.joint(p=[-65.8, 170 / 2. + 10, 58 * i]),
           mc.joint(p=[-65.8 + 20, 170 / 2. + 10, 58 * i]), mc.joint(p=[23.8, 170 / 2. + 10, 58 * i]),
           mc.joint(p=[23.8 - 20, 170 / 2. + 10, 58 * i]), mc.joint(p=[65.8, 170 / 2. + 20, 58 * i]),
           mc.joint(p=[65.8 - 20, 170 / 2. + 20, 58 * i]), mc.joint(p=[65.8, 170 / 2., 58 * i]),
           mc.joint(p=[65.8 - 20, 170 / 2., 58 * i])]
    mc.select(kho[0])
    mc.select(kho[0])
    mc.select(kho[0])
...
kraduk = [kraduk_natang[0], kraduk_pratu[0], kraduk_tulinchak[0][0], kraduk_tulinchak[1][0], kraduk_tusueapha[0][0],
          kraduk_tusueapha[1][0]]
for n in g:
